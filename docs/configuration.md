...
| `KINETIC_OUTPUT_DIR`         | Library (submit) + remote pod | `gs://{bucket}/outputs/{job_id}` | Per-job durable artifact prefix. Kinetic reads it at submit time and sets it in the pod. There is no CLI flag. See [Checkpointing](guides/checkpointing.md).                                                  |
