...
class TestSubmitDecorator(absltest.TestCase):
  def test_decorator_preserves_wrapped_function(self):
    @submit(accelerator="cpu")
    def my_func():
      """My docstring."""
...
class TestEnvVarCapture(absltest.TestCase):
  def test_exact_match(self):
    with (
      ...
      mock.patch(
        "kinetic.core.core._execute_on_backend"
      ) as mock_exec,
    ):
      ...
      # env_vars is third-from-last positional arg
...
class TestSubmitOnBackend(absltest.TestCase):
  def test_submit_calls_submit_remote(self):
    """submit() decorator calls submit_remote() and returns the handle."""
    mock_handle = MagicMock()
    with (
