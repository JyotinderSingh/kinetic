...
class JobHandle:
  ...
  def _get_status_name(self) -> str:
    """Return the backend-native status string for this job."""
  ...
  def _missing_result_error(self, status: JobStatus) -> RuntimeError:
    ...
    if status == JobStatus.FAILED:
      return RuntimeError(
        "Job failed but no result payload was found at " f"{result_uri}"
      )
    return RuntimeError(
      "Job completed but no result payload was found at " f"{result_uri}"
    )
  ...
  def status(self) -> JobStatus:
    ...
    return JobStatus(self._get_status_name())
