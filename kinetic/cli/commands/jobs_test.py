...
class TestJobsResult(absltest.TestCase):
  ...
  def test_result(self, mock_attach):
    ...
    handle.result.assert_called_once_with(timeout=None, cleanup=True)
  ...
  def test_result_no_cleanup(self, mock_attach):
    ...
    self.assertEqual(result.exit_code, 0)
    handle.result.assert_called_once_with(timeout=None, cleanup=False)
  ...
  def test_result_with_timeout(self, mock_attach):
    ...
    self.assertEqual(result.exit_code, 0)
    handle.result.assert_called_once_with(timeout=300.0, cleanup=True)
...
class TestJobsCleanup(absltest.TestCase):
  ...
  def test_cleanup_both(self, mock_attach):
    ...
    self.assertEqual(result.exit_code, 0)
    handle.cleanup.assert_called_once_with(k8s=True, gcs=True)
  ...
  def test_cleanup_gcs_only(self, mock_attach):
    ...
    self.assertEqual(result.exit_code, 0)
    handle.cleanup.assert_called_once_with(k8s=False, gcs=True)
  ...
  def test_cleanup_k8s_only(self, mock_attach):
    ...
    self.assertEqual(result.exit_code, 0)
    handle.cleanup.assert_called_once_with(k8s=True, gcs=False)
